...
import timeFomate from '../lib/TimeFormat';

const MovieCard = ({movie}) => {
    const navigate = useNavigate();
  return (
    <div className='flex flex-col justify-between p-3 bg-gray-800 rounded-2xl hover:-translate-y-1 transition duration-300 cursor-pointer w-60'>
        ...
        <p className='text-sm text-gray-400 mt-2'>
            {new Date(movie.release_date).getFullYear()} * {movie.genres.slice(0,2).map(genre=>genre.name).join(" | ")} * {timeFomate(movie.runtime)} min
        </p>
